...
Ps2Core::Ps2Core(uint32_t core_base_addr) { base_addr = core_base_addr; }
...
int Ps2Core::rx_byte() {
  ...
  if (rx_fifo_empty()) // no data
    ...
    io_write(base_addr, RM_RD_DATA_REG,
             0); // dummy write to remove data from rx FIFO
    return ((int)data);
  }
}
...
int Ps2Core::get_mouse_activity(int *lbtn, int *rbtn, int *xmov, int *ymov) {
  uint8_t b1, b2, b3;
