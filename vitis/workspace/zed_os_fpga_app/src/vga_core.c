...
{
    fb_write(y * HMAX + x, 0x00000000);
}

void setpix(int x, int y)
{
    unsigned int c = (color >= 0 && color <= WHITE) ? color_table[color] : 0x00FFFFFF;
    fb_write(y * HMAX + x, c);
}
...
static void scroll(void)
{
    ...
    for (i = 0; i < HMAX * VMAX - HMAX * 16; i++)
        fb_write(i, *(volatile unsigned int *)(VIDEO_FRAME_BASE + 4 * (i + HMAX * 16)));
    // blank the last row
...
static void unkpchar(char c, int ro, int co)
{
    undchar(c, co * 8, ro * 16);
}
...
int kputc(char c)
{
    ...
    if (c == '\n') {
        row++;
        if (row >= 25) { row = 24; scroll(); }
        putcursor();
        return 0;
    }
    if (c == '\b') {
        if (col > 0) { col--; erasechar(); }
        putcursor();
        return 0;
    }
    kpchar(c, row, col);
