module ps2tx
   (
    ...
    // PS/2 clock - split tristate (IOBUF at top level)
    input  logic ps2c_in,
    output logic ps2c_out_o,
    output logic ps2c_t,
    // PS/2 data - split tristate (IOBUF at top level)
    input  logic ps2d_in,
    output logic ps2d_out_o,
    output logic ps2d_t
   );
   ...
   logic tri_c, tri_d;
   logic ps2c, ps2d;  // internal aliases from IOBUF inputs
   assign ps2c = ps2c_in;
   assign ps2d = ps2d_in;
   ...
   // tristate control outputs - T=1 means high-impedance (input mode)
   assign ps2c_out_o = ps2c_out;
   assign ps2c_t     = ~tri_c;   // tri_c=1 -> drive low -> T=0; tri_c=0 -> release -> T=1
   assign ps2d_out_o = ps2d_out;
   assign ps2d_t     = ~tri_d;
endmodule